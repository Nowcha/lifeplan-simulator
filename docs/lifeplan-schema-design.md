...
interface Child {
  ...
  /** 合計所得金額(年額)。アルバイト等で扶養を外れる/特定親族特別控除に切り替わる判定に使う */
  annualIncome?: Yen;
}
...
- **所得要件**: 合計所得金額が `dependentDeduction.incomeMax`(令和8年分62万円)以下の者だけが扶養親族に当たる。超える者は控除も非課税限度額の人数算入も受けない
- **特定親族特別控除**(令和7年度創設): 19〜22歳の親族の合計所得が要件を超えて123万円以下のとき、外れた特定扶養控除の代わりに段階的な控除を与える(所得税63〜3万円、住民税45〜3万円)。扶養控除とは排他 — 所得要件を超えた時点で扶養控除は0になるため、二重には足されない。子の所得は `Child.annualIncome` で指定する
...
### 4-2. 配偶者特別控除の扱い(Phase 2 追加)

世帯は2人まで持てるため、片方が扶養の範囲を少し超えて働くケース(いわゆる「136万円の壁」)が現実的に起こる。配偶者控除だけを実装した状態では、配偶者の合計所得が要件(令和8年分: 62万円)を1円超えた瞬間に控除がゼロに落ちる崖ができ、税額を過大に見積もる。

- **適用範囲**: 配偶者の合計所得金額が `spouseDeduction.spouseIncomeMax`(62万円)超 133万円以下。本人の合計所得金額が1,000万円を超える場合は適用なし
- **控除額**: 配偶者の合計所得の帯 × 本人の合計所得の帯(900万/950万/1,000万)の2次元テーブル。所得税と住民税で金額が異なる(住民税は上限33万円)
- **配偶者控除との排他**: 両者は同時に適用されない。配偶者の所得が要件以下なら配偶者控除、超えて133万円以下なら配偶者特別控除
- **調整控除**: 配偶者特別控除は**人的控除額の差の対象外**(江東区の一覧で全帯「適用無」)。配偶者控除のときだけ差が乗るため、エンジンは両者を種別として区別して返す

**老人控除対象配偶者**: 配偶者がその年12/31時点で70歳以上なら控除額が割増になる(所得税48/32/16万円、住民税38/26/13万円、調整控除の人的控除差10/6/3万円)。配偶者は `Person` なので `birthYearMonth` から年齢を導出でき、スキーマ拡張は不要。配偶者特別控除の帯には割増が無い。
