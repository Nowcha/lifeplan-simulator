...
import { runSimulation, type SimulationBundle } from './lib/engine'
...
const UI_SCALE_NOTE = '計算量の都合で縮小したパス数(本番設定は10,000パス)で実行しています。'
...
export default function App() {
  ...
  useEffect(() => {
    // ローディング表示を先に描画させてからメインスレッドをブロックする計算に入る
    const timer = window.setTimeout(() => setResult(runSimulation()), 0)
      ...
      <header className="border-b border-hairline bg-surface">
        <div className="mx-auto max-w-5xl px-6 py-8">
          ...
          <p className="mt-2 max-w-2xl text-sm text-ink-secondary">
            {firstRow?.year}年〜{finalRow?.year}年、{rows.length}年間の家計を試算。モンテカルロは{UI_SCALE_NOTE}
          </p>
        </div>
      </header>
