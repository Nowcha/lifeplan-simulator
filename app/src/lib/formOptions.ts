...
export const CHILD_INCOME_HELP =
  'アルバイト等による子本人の年間の合計所得金額(給与収入から給与所得控除を引いた後の金額)。62万円(令和8年分)を超えると扶養控除が外れるが、19〜22歳なら特定親族特別控除に切り替わるため控除が急に消えることはない。123万円を超えると控除なしになる。'
